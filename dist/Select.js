...
const renderSelect = (control) => {
    ...
    return (react_1.default.createElement(react_bootstrap_1.Form.Control, Object.assign({ key: `${name}-${id}` }, control, { as: 'select', multiple: multiple }), options.map((option, index) => react_1.default.createElement("option", { key: `${name}-${id}-option-${index}` }, option))));
};
